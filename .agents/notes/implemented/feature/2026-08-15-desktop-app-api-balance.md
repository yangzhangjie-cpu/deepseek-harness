...
The WebView reserves the middle of its top 32 px as a native drag region because full-size WebKit content otherwise consumes the transparent titlebar's draggable background. Left navigation and right settings controls remain outside the region; a double click retains the standard macOS window zoom behavior.
